...
skill trees. Category order applies to every renderer; the `skills` `bridge` operations are
applied only to the web's concise default projection, while non-web artifacts retain the
canonical Skills baseline.
