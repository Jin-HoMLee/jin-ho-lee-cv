...
**Architecture (Phase 8b scope):** A new `target` axis sits orthogonal to the existing `lang` axis, resolved once inside `content_loader.load_content` *before* langstring resolution. A target variant overrides only positioning fields (headline, tagline, lead paragraph, featured-project order); `bridge` is the strict canonical fallback. Because the loader strips all variant structure and returns the same flat tree it returns today, every downstream renderer is unchanged. PDF + plain-text gain `--target`; JSON Resume, JSON-LD, and the entire web layer stay canonical `bridge`. Phase 8c later extended the shared model with Skills category ordering and projections for web data and non-bridge targeted PDFs; see the [current Phase 8c design contract](../specs/2026-05-31-phase-8c-web-variants-design.md).
