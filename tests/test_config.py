...
def test_site_domain_is_bare_host():
    ...
    assert re.match(r"^[a-z0-9.-]+$", SITE_DOMAIN), (
        f"unexpected chars in SITE_DOMAIN: {SITE_DOMAIN!r}"
    )
...
def test_pages_base_url_format():
    ...
    assert PAGES_BASE_URL.startswith("https://"), (
        f"PAGES_BASE_URL must be https (got {PAGES_BASE_URL!r})"
    )
    assert not PAGES_BASE_URL.endswith("/"), (
        f"PAGES_BASE_URL must not end with '/' (got {PAGES_BASE_URL!r})"
    )
