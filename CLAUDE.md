...
| 14 | AEO entity presence (Google Scholar entity anchor · bilingual FAQ + `FAQPage` JSON-LD · front-loaded answer block · static-HTML-facts CI guard) | 🔶 Phase work complete 2026-07-14 on `phase-14-aeo-entity-presence`; not yet merged (fill in `✅ Done (merged YYYY-MM-DD, --no-ff, PR #___)` here once the PR is opened and merged). Google Scholar is wired into `personal.links` and flows into JSON-LD `sameAs`; a Wikidata item is a documented human follow-up, not yet created - see `docs/runbooks/2026-07-wikidata-entity.md` |
